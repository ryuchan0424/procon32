...
def get_goal_array():
    ...
    for i2 in range(len(y_wari)):
        ...
        for i1 in range(len(x_wari)):
            ...
            in_array.append([[int(data[0]), int(data[1])]])
            rot.append(int(data[2]))
    
    for i in in_array: out_array.append(XY_index(i[0], i[1]))
...
def get_first_direction(now, next):
    ...
    # ゴール一つ前のの盤面
    now_array = copy.copy(now['array'])
    now_index = now_array.index(position)
    x, y = XY_coord(now_index)

    # ゴール盤面
    next_array = copy.copy(next['array'])
    next_index = next_array.index(position)
...
def move(now_array, goal_number, confirm_array):
    ...
    start_array = copy.copy(now_array)
    goal_array = copy.copy(goal_board)
    r_start_array = copy.copy(now_array)
    r_goal_array = copy.copy(goal_board)
    free_number = width * height + 1 # 自由に動かせるマスの番号

    confirm_array_index = []
    for i in confirm_array:
        index = start_array.index(i) # 指定したマスのindex
        confirm_array_index.append(index)

    # 現在の盤面
    index = start_array.index(goal_number) # 指定したマスのindex
    start_array = [free_number] * (width * height)
    start_array[index] = goal_number # 指定したマスを元の値に戻す

    confirm_array_index2 = []
    for i in range(len(confirm_array)):
        index2 = confirm_array_index[i]
        start_array[index2] = confirm_array[i]

        index = goal_array.index(confirm_array[i])
        confirm_array_index2.append(index)

    # ゴール盤面
    goal_array.index(goal_number) # 指定したマスのindex
    goal_array = [free_number] * (width * height)
...
def move_board(board_array, position, root):
    now_array = copy.copy(board_array)
    
    position = now_array.index(position)
...
def create_array(array):
    out_array = copy.copy(array) # 出力配列
    index = out_array.index(position) # 0の位置
    out_array.remove(out_array[index]) # 指定したindexを削除
    # print('pos', position)
...
def counter(start, goal):
    counts = 0
    start_array = copy.copy(start)
    goal_array = copy.copy(goal)

    for i in range(len(start_array)):
        if start_array[i] != goal_array[i]:
            index = start_array.index(goal_array[i]) # 位置
